...
- A source is admitted only after exact cwd, compatible call-path/input/checkpoint formats, complete resume metadata, and valid manifest/seed checks. For a normally settled source, the recorded start and terminal environments must prove intra-run filesystem stability. Git recordings use HEAD plus the dirty digest; non-Git recordings use `environmentKey`. The current environment and current Node/V8 versions are provenance, not admission facts. A safe, start-to-terminal-stable source can use identity matching; an unsafe stable new-format source gets only a safety-checked positional prefix, while nested or source-internally-drifted fallback is all-live. `replayEligibility.provenanceChanges` reports recorded-terminal/current environment differences and source/current Node/V8 differences alongside the non-gating engine-version diagnostic.
...
- `resumePolicy: "positional"` is the migration escape hatch: it requests index/prefix correspondence but cannot bypass new-format format, metadata, manifest, cwd, input, or safety checks. Marker-less journals and permanently marked manual/same-run legacy resumes retain historical hash-only positional behavior. Reconciled `paused` / `interrupted` crash snapshots use `crash-residue` with `legacy` prefix eligibility before the input-format bridge; their recorded start-environment difference from the current world is reported without changing that eligibility. Normally settled marked sources below input format 2 use `inputs-format-legacy`. Ancestor-scoped rows carried by a ≤0.23 resume hop replay only while that ancestor is still persisted, and nested/deleted scopes stay live.
...
An all-live outcome is expected calibration, not an engine error. Missing resume metadata, incompatible format literals, an invalid manifest/seed, or unproved recorded start-to-terminal filesystem stability can disable reuse or make a fallback all-live. A new-format source containing any result row without a captured call path/input fact—possible with a call stack deeper than the raw-frame cap or a non-strict-JSON `meta` value—is source-wide `"manifest-invalid"`; excluding the row could make an ambiguous sibling look unique. Current-world environment drift and Node/V8 upgrades never cause all-live. Format-1 bytes are never reinterpreted; they enter the positional bridge and replayed rows are recorded under format 2.
