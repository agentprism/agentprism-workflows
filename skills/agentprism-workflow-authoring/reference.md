...
- A source is admitted only after exact cwd, full Node/V8/compatible runtime-format, and terminal environment checks. Git sources compare HEAD plus the dirty digest; non-git hosts must provide the same `environmentKey`. Unknown future formats, missing facts, source drift, and uncertainty select all-live. A safe, stable source can use identity matching; an unsafe but stable new-format source gets only a safety-checked positional prefix, while nested or source-drifted fallback is all-live. The producing engine package version is surfaced but never gates admission.
