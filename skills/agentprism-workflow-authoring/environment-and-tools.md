...
- `mode` requests an exact agent-advertised ACP session mode. Config returns the backend's raw names, descriptions, and `_meta`. When omitted, AgentPrism applies Claude `auto`, Codex `agent`, OpenCode `build`, or no Pi mode; authored and built-in defaults are validated before prompting. Only set `mode` on calls whose `model` you also pin. Use an explicitly advertised read-only/plan mode for reviewers and auditors that must not write.
