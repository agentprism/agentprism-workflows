...
- `mode` requests an exact agent-advertised ACP session mode. Config returns the backend's raw names, descriptions, and `_meta`. When omitted, AgentPrism applies Claude `auto`, Codex `agent`, OpenCode `build`, or no Pi mode; authored and built-in defaults are validated before prompting. Only set `mode` on calls whose `model` you also pin. For trusted autonomous implementation/review workflows, explicitly use Claude `bypassPermissions` or Codex `agent` when advertised. Claude `auto` uses a model classifier and may request permission; it is not fully autonomous. Use an explicitly advertised read-only/plan mode for reviewers and auditors that must not write.
