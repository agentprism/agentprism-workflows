...
- Source admission requires exact `cwd`, compatible call-path/input/checkpoint fingerprint formats, complete metadata, and a valid manifest/seed. For a normally settled source, the recorded start and terminal environments must prove that its filesystem stayed stable while it ran; Git identity is HEAD plus dirty digest, while non-Git hosts use `environmentKey`. The current environment and current Node/V8 versions do not gate admission. Differences from the source's recorded terminal environment and runtime are reported in `replayEligibility.provenanceChanges` so the agent can judge a world that moved. A safe, start-to-terminal-stable source gets identity replay; an unsafe stable source can get only a safety-checked positional prefix; nested or source-internally-drifted fallback is all-live. Producing/current workflow-engine versions are also diagnostics, never gates.
...
- `resumePolicy: "positional"` is a migration escape hatch for index/prefix matching, not permission to bypass new-format format, metadata, manifest, cwd, input, or safety checks. Marker-less/manual/same-ID legacy journals keep historical hash-only positional behavior. A crash snapshot reconciled to `paused` / `interrupted` with an identity marker but no quiescent terminal environment uses the named `crash-residue` bridge and admits its hash-stable positional prefix as `legacy`; any recorded start-environment difference from the current world is provenance only. A marked, normally settled source with input-fingerprint format below 2 uses the `inputs-format-legacy` hash-only positional bridge; replayed rows are written under format 2 for the next hop. A carried prefix from a ≤0.23 resume hop is eligible when each row is unscoped, scoped to the immediate source, or scoped to an ancestor run still persisted beside it. Engine-minted nested scopes and deleted ancestor scopes are excluded.
...
Every `resumeFromRunId` result has a bounded `replayEligibility` summary. MCP background admission, foreground completion, both await shapes, and inspect expose the same strategy, predicted replayable-prefix length, observed replayed prefix/counts, and first non-replay when one is known. The first reason is either the admission fallback/disabled reason or one of `strategy-live`, `positional-miss`, `positional-suffix`, `not-recorded`, `path-missing`, `inputs-missing`, `inputs-changed`, `ambiguous-identity`, `ambiguous-content`, `candidate-consumed`, `empty-output`, `safety-changed`, `unsafe-suffix`, `worktree-degraded`, `seed-persistence-error`, or `resume-fatal-latch`. A derivable detail names the differing admission fact or operational values. Engine/input-format versions and `provenanceChanges` for current-world environment, Node, and V8 drift appear alongside it as diagnostics.

An all-live outcome is expected calibration, not an engine error. Missing resume metadata, incompatible format literals, or an invalid manifest/seed can disable new-format replay. If any source result lacks a captured path/input fact—possible when a call stack exceeds the raw-frame cap or `meta` is not strict JSON—the whole source is `"manifest-invalid"`; dropping that row could make an ambiguous sibling look unique. Recorded start-to-terminal environment instability remains a conservative safety fact and can make positional fallback all-live, but a difference between the recorded source world and the current world cannot. Crash residue always enters its `"crash-residue"` / `"legacy"` bridge. Node/V8 drift is reported provenance. Identity-v1 fingerprint bytes are never reinterpreted: normally settled format-1 sources enter the input-format bridge, and a format greater than the current format is `"runtime-mismatch"`.
...
`replayEligibility` and full `resumeReport` instead of assuming a prefix hit.
