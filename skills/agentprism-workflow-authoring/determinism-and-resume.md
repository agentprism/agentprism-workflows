...
- Source admission requires exact `cwd`, full Node/V8 and compatible fingerprint formats, and an environment captured after the source settled with no engine-known work outstanding. Git identity is HEAD plus dirty digest; non-git hosts must provide the same `environmentKey`. A safe stable source gets identity replay; an unsafe stable source can get only a safety-checked positional prefix; nested/source-drifted fallback and every invalid/uncertain source are all-live. Journals resume forward across workflow-engine package versions: the producing version is reported for diagnosis but a version difference never gates admission.
...
- `resumePolicy: "positional"` is a migration escape hatch for index/prefix matching, not permission to bypass new-format input, safety, cwd, runtime, or environment gates. Marker-less/manual/same-ID legacy journals keep historical hash-only positional behavior. A crash snapshot reconciled to `paused` / `interrupted` with an identity marker but no quiescent terminal environment uses the named `crash-residue` bridge: it replays the hash-stable positional prefix only when the source's admission environment equals the current environment, and reports `all-live` otherwise. A marked, normally settled source with input-fingerprint format below 2 uses the `inputs-format-legacy` hash-only positional bridge; replayed rows are written under format 2 for the next hop. A carried prefix from a ≤0.23 resume hop is eligible when each row is unscoped, scoped to the immediate source, or scoped to an ancestor run still persisted beside it. Engine-minted nested scopes and deleted ancestor scopes are excluded.
...
Every `resumeFromRunId` result has a bounded `replayEligibility` summary. MCP background admission, foreground completion, both await shapes, and inspect expose the same strategy, predicted replayable-prefix length, observed replayed prefix/counts, and first non-replay when one is known. The first reason is either the admission fallback/disabled reason or one of `strategy-live`, `positional-miss`, `positional-suffix`, `not-recorded`, `path-missing`, `inputs-missing`, `inputs-changed`, `ambiguous-identity`, `ambiguous-content`, `candidate-consumed`, `empty-output`, `safety-changed`, `unsafe-suffix`, `worktree-degraded`, `seed-persistence-error`, or `resume-fatal-latch`. A derivable detail names the differing admission fact or operational values; engine versions and input-format versions appear alongside it as diagnostics.

An all-live outcome is expected calibration, not an engine error. If any source result lacks a captured path/input fact—possible when a call stack exceeds the raw-frame cap or `meta` is not strict JSON—the whole source is `"manifest-invalid"`; dropping that row could make an ambiguous sibling look unique. Crash residue with an unknown or drifted admission environment is `"crash-residue"` / `"all-live"`, never fabricated terminal proof. A Node or V8 upgrade invalidates identity replay through exact runtime equality; marker-less historical positional journals do not depend on those runtime fields. Identity-v1 fingerprint bytes are never reinterpreted: normally settled format-1 sources enter the input-format bridge, and a format greater than the current format is `"runtime-mismatch"`.
