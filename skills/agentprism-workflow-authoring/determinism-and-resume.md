...
- Host `agentRetries` and `concurrency`, plus per-call `retries`, are operational bounds. They enter neither hash and may change freely on resume. A new run resolves them from its own request; it does not inherit the source values.
