...
test("public package inventories cover every workspace package", () => {
  ...
  assert.equal(manifests.length, 6, "update the documented package-count contract when the workspace changes");
  ...
  assert.match(contributing, /\(monorepo\) of six packages/);
  assert.match(readRepoFile("docs/design-notes.md"), /monorepo of \*\*six\*\* published packages/);
});
