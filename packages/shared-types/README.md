...
- `WorkflowRunInspectionOptions`, `WorkflowLogTail`, `WorkflowRunCallStatus`,
  ...
  by SDK and MCP polling/inspection hosts. Agent call status can carry a terminal `errorCode`,
  including `AGENT_CANCELLED` for
...
- `WorkflowRunLimits` — resolved `maxAgents`, `concurrency`, and `agentRetries`; it is returned as
  `WorkflowRunResult.effectiveLimits` and as
