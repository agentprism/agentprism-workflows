...
- **Lifetime**: only signals, `daemon stop`, sustained idleness (default: 15 min with zero sessions and zero active runs, `AGENTPRISM_DAEMON_IDLE_TTL_MS`, `0` disables), or supersession (see above — a superseded daemon exits as soon as nothing is busy, regardless of the idle TTL) end the daemon. Client disconnects never cancel runs — per the Streamable HTTP spec, cancellation is only an explicit MCP cancellation. Dead-client sessions (no open connections, no traffic for `AGENTPRISM_SESSION_TTL_MS`, default 5 min) are evicted without touching their runs; the shim transparently re-initializes on the spec's 404, so eviction, daemon restarts, and upgrades are invisible to live clients. The REPL client-presence drain has its own bound, `AGENTPRISM_REPL_DRAIN_BOUND_MS` (default 2 h). A request that is in flight when its session is lost (daemon crash) is answered by the shim with a JSON-RPC error instead of hanging.
