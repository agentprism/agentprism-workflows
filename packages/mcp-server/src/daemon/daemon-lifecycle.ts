 ...
 *   - every session with no request in flight is closed so its
 ...
 *   - the moment nothing is busy — no sessions, runs, or requests — the daemon exits,
...
export function installDaemonLifecycle(options: DaemonLifecycleOptions): DaemonLifecycle {
  ...
  const reaper = setInterval(() => {
    ...
    if (superseded) {
      ...
      // session independently; in-flight requests remain protected by SessionRegistry's
      // eviction predicate.
    ...
    // Idleness means no sessions, no active workflow runs, and no request in flight.
