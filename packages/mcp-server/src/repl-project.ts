 ...
 *  ref ids and read back through a later call's `refs` parameter.
 *
 *  ROUND 3 — the refs are WORKSPACE-NAMESPACED and RETENTION IS
 *  UNBOUNDED (until `clear`):
 *  - Every ref id carries the workspace's project key prefix
 *    (`<key>:t<seq>` — the same key the per-project store derives from
 *    the canonical projectDir), so a ref obtained from one project can
 *    never resolve in another context's store: the projectless `status`
 *    search finds exactly the owning workspace (a ref from project A
 *    can never silently return project B's data — the reviewer's
 *    cross-project leak).
 *  - No ring eviction: an advertised ref must stay readable — "the cap
 *    costs reads, never data" — and the ephemeral completion/output
 *    tails cannot be regenerated by re-reading current state. The
 *    store is in-memory and per-workspace, each snapshot is bounded by
 *    the data the cap actually elided, and `reset` clears it (see
 *    `resetReplProjectState`), so retention is unbounded by design.
 *  - The sequence is store-global, so chained reads never collide with
 *    earlier ids (a re-capped read-back's fresh ref continues the
 *    chain). */
...
export interface ReplProjectState {
   ...
   *  fresh ref id that the result's `truncated` record carries, and a
   *  later eval/wait/status call's optional `refs` parameter reads them
   *  back — the cap costs reads, never data (the doc's `$N` principle
   *  extended to every omitted structured field). Refs are
   *  WORKSPACE-NAMESPACED (`<project-key>:t<seq>` — a ref from one
   *  project can never resolve in another context) and retained until
   *  `reset` clears the store (round 3: no ring eviction — an
   *  advertised ref must stay readable; the store is in-memory and
   *  per-workspace, and a daemon restart re-reads current state with
   *  fresh elisions and fresh refs). */
...
export async function resetReplProjectState(
  ...
  // The dropped workspace's truncation refs die with it (phase-F review
  // round 3: reset used to leave `state.truncationRefs` populated — old
  // workspace metadata remained retrievable through the `refs`
  // parameter after the tool reported the workspace state was dropped).
