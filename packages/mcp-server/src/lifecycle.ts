...
export interface WorkflowServerControl {
  ...
  /** Release state the serving entry shares across server instances (event subscriptions, the
   *  permission broker). Runs once at shutdown, after the runner is disposed. OPTIONAL: a server
   *  that owns nothing of the kind omits it. */
  ...
  const lifecycle: McpServerLifecycle = {
    shutdown(reason) {
      ...
      shutdownPromise = disposeRunnerWithDeadline(options.runner, deadlineMs).then(() => {
        removeListeners();
        // Fire-and-forget: the exit below is authoritative.
        void options.server.dispose?.().catch(() => undefined);
