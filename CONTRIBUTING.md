...
> The Codex backend's turn-level `outputSchema` forward is baked into the workspace `@automatalabs/codex-acp` package (`packages/codex-acp`, consumed by `acp-agents` as `workspace:*`), so there is nothing to patch locally.
...
- The `AGENTPRISM_*` env vars, the `.agentprism/` runtime dirs, and the `agentprism-workflow` bin are a **wire/CLI contract** — they are intentionally *not* renamed with the npm scope. The workflow ACP dialect's protocol-critical `_meta` keys are **bare** (un-namespaced): `outputSchema`, `runId`, `systemPrompt` (the Claude / pi session system-prompt channel), `baseInstructions`, `developerInstructions` (the Codex channel for the same `systemPrompt` option) — exported as `META_KEYS` / `CODEX_META_KEYS` from `shared-types`. The ACP proxy's negotiated routing extension instead lives under `_meta["@automatalabs/agentprism"].acpRouter`. Beyond the reserved keys, workflows can send **arbitrary** `_meta` via `agent({ meta, promptMeta })` (session/new / session/prompt scoped), and **custom ACP backends** register via `AGENTPRISM_BACKENDS` or `createAcpRunner({ backends })` — see `acp-agents/src/registry.ts`.

### Existing implementations are not the design authority

Design from first principles for the request at hand; the rule and its constraints are in [`AGENTS.md`](AGENTS.md). Current behavior is documented by the code, its tests, this file, the READMEs, `docs/api.md`, and `docs/authoring/`. Nothing here is frozen: change architecture, contracts, and the tests that enshrine them when that produces a better outcome, and land the migration, tests, and docs in the same change.

`docs/archive/` holds historical design records from past implementation trains. They are kept for provenance only: not maintained, not an authority, and not to be read unless someone points you at one.
...
- **Canonical MCP authoring skills → generated Skills Extension bundle.** `docs/authoring/manifest.json` explicitly allowlists the workflow Agent Skill directory under `docs/authoring/`. After ANY edit there, regenerate `packages/mcp-server/src/generated/authoring-skills-content.ts` and commit source plus generated output together. A normal `pnpm build` runs `scripts/ensure-authoring-skills.mjs` before either published consumer builds, so agents do not need to remember a separate step; `pnpm generate:authoring-skills` remains the explicit command. `packages/mcp-server/test/authoring-skills.test.ts` byte-compares the generated module, enforces Agent Skills frontmatter/path rules and project/SEP bounds, verifies the public workflow API vocabulary, and rejects REPL, SDK-host, and CLI wording that an agent driving the MCP tool cannot act on. The `author-workflow` prompt is intentionally compact and hand-maintained in `packages/mcp-server/src/authoring-prompt.ts`; it directs the host to activate the workflow skill without embedding it or bypassing host approval.
...
- **Published ACP servers → GitHub Pages registry.** `scripts/generate-acp-registry.mjs` owns the authored metadata for the public ACP-format registry, and `docs/assets/agentprism-mark.svg` plus entry-specific SVGs provide its validated 16×16 icon sources. The generator discovers every public in-repo `@automatalabs/*-acp` package plus the `@automatalabs/acp-server` aggregation entry point, and fails if its allowlist omits one; it also requires npm `latest` to equal the checked-in package version before writing anything. With `--wait-for-publish <seconds>` it first polls (every `AGENTPRISM_ACP_REGISTRY_POLL_MS` milliseconds, default 30000) while `latest` is semver-behind the checkout, then applies that same exact-version check; a `latest` that is ahead of the checkout is refused immediately, and without the flag every mismatch is refused immediately. `.github/workflows/npm-download-badge.yml` generates the registry and npm-download documents into one complete Pages artifact. Its push trigger skips the `Version Packages (#NNN)` merge commit (npm has not published those bumped versions yet, so that run could only refuse); the `workflow_run` trigger after a successful Release workflow is the refresh for that release and passes `--wait-for-publish 900`, because npm's read path serves the previous `latest` for minutes after publication. Scheduled, manual, and other push runs pass no wait. The download generator discovers every public in-repo `@automatalabs/*` package and reports an explicit 30-day UTC window ending on the most recent settled day at least two days before the run: npm reports a day it has not aggregated yet as zero rather than as missing, so when the aggregate for the final day is zero the generator steps the window end back one day at a time (to at most four days before the run), re-fetching every package for the same period, and records the lag it used as `period.reportingLagDays`; a window whose final day is still zero at that bound is refused and nothing is written. When npm's full range has no row yet for a new package, registry metadata determines its exact pre-creation days: a package created after the window contributes zero, while one created inside the window is queried from its creation day and the earlier days are zero-padded. Post-creation rows retain the same strict completeness checks. Keep those outputs coupled because a Pages deployment replaces the prior artifact. Run `pnpm test:scripts` after changing the generator, icon, package discovery convention, or Pages wiring.
- **Dependency pins → welded tests and docs.** A dependency bump moves, at minimum: the exact-pin map in `packages/pi-acp/test/packaging.test.ts`; `FIXTURE_PI_PIN` plus the cited fixture strings in `packages/pi-acp/test/fixtures/provider-error-strings.ts` (re-verify each string against the new version's installed dists before advancing the pin); `@earendil-works/pi-agent-core` in pi-acp's devDependencies, which moves in lockstep with the pi runtime **even though the gate output does not list it**; and installed-version citations in `docs/api.md`. The docs-drift suite (`packages/acp-agents/test/docs-drift.test.ts`) also welds the event-name tables (the workflows and acp-agents READMEs plus `docs/api.md`), the mcp-server README contract tokens, and this file's own package inventory. Fix what the guards name; never loosen a guard to make an edit pass.
...
- **Pi dependency packaging smoke.** After changing Pi runtime dependencies, run `node scripts/smoke-pi-package.mjs`. It builds and packs pi-acp, installs the tarball into fresh npm and pnpm projects, and imports its public SDK. This networked check stays separate from the deterministic default suite and catches dependency omissions hidden by workspace installs.
