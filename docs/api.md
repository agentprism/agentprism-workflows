...
Installed backend status verified from the packaged dists: `@agentclientprotocol/claude-agent-acp@0.59.0` advertises `http`/`sse` MCP support but no `acp`, `@automatalabs/codex-acp@1.6.5` advertises `mcpCapabilities: { acp: false, http: true, sse: false }` and rejects ACP MCP config internally, OpenCode advertises HTTP/SSE MCP support, and `@automatalabs/pi-acp` advertises `mcpCapabilities: {}` because it serves stdio MCP only. Current MCP-over-ACP integration tests therefore use the repository fake agent fixture.
