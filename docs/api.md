...
**The `author-workflow` prompt.** Prompt-capable hosts additionally get one user-controlled MCP prompt, `author-workflow` (optional `task` argument): it returns the complete, self-contained protocol-native authoring guide — generated from SKILL.md + the exhaustive reference tables + a validated example script, with terminal-only command sections omitted by `scripts/generate-authoring-prompt.mjs`, and version-matched to the installed engine. Prompts never enter the model's tool-selection loop, so the prompt adds no tool to the model-facing surface (`workflow` and `repl`).
