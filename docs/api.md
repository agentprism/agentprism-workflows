...
`npx @automatalabs/mcp-server` (bin `agentprism-workflow`) speaks stdio MCP and exposes three model-facing tools: selective **`docs`**, deterministic **`workflow`**, and persistent interactive **`repl`**. By default the stdio process is a thin shim proxying to the shared per-user workflow daemon (Streamable HTTP on loopback, auto-started, spec 2025-11-25 session management and resumability); `--in-process` serves everything in the one stdio process instead, and HTTP-capable hosts can register the daemon URL directly (`agentprism-workflow daemon url`). The tool contract is identical on every path except one knob: the daemon **requires** `projectDir` on workflow config/run and REPL inputs, while an in-process server defaults it to its own project.

`docs({ topic? })` reads one bundled, version-matched authoring topic. `topic` is a closed enum; omission defaults to the bounded `index`. The result's structured metadata is `{ topic, title, description, uri, mimeType, relatedTopics, bytes }`, and its content contains the exact selected `text/markdown` embedded resource. The same bytes are available through `resources/read` at `agentprism://docs/<topic>`. The index is capped at 8 KiB and every other topic at 16 KiB. Calls need no project, run no code, open no backend, and persist nothing.
