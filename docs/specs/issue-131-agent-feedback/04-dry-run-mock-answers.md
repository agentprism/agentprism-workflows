...
The opt-in serialization also admits each mock call only after the previous one settles through the FIFO limiter, where today's concurrent wave may be admitted before any of those calls settle. That scheduling change is the cost of deterministic sequence allocation and is confined to dry runs that supplied `mockAnswers`; the report and docs must describe it so authors do not treat the validator as a concurrency/load simulator.
