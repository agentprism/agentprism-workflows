...
| **PR5** | MCP server auth tools (§4.3) | `packages/mcp-server/src/server.ts`, new `auth-tool-io.ts`, new `auth-resolver.ts`, `packages/mcp-server/test/auth-tools.test.ts` | Three additive tools + summary branch; `createWorkflowServer` signature unchanged; inline elicitation is env-gated OFF. |
